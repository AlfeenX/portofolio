...
const Contact = () => {
  ...
  useGSAP(() => {
    ...
    tweenRef.current = gsap.to(".marquee-group", {
      x: -groupWidth,
      ease: "none",
      duration: 20,
      repeat: -1,
  ...
  return (
    <section id="contact" className="h-screen w-full relative overflow-hidden gap-0 px-4 flex flex-col">
      ...
      <div 
        ...
        className="absolute bottom-0 left-0 w-full overflow-hidden bg-white py-4 backdrop-grayscale-75 cursor-pointer"
      >
        <div className="flex w-max select-none">
          
          <div ref={groupRef} className="marquee-group flex items-center gap-20 pr-20 shrink-0">
            {Techs.map((tech) => (
              <img key={`g1-${tech.id}`} src={tech.image} alt={tech.name} className="h-12 w-auto object-contain" />
            ))}
          </div>

          <div className="marquee-group flex items-center gap-20 pr-20 shrink-0" aria-hidden="true">
            {Techs.map((tech) => (
              <img key={`g2-${tech.id}`} src={tech.image} alt={tech.name} className="h-12 w-auto object-contain" />
            ))}
          </div>

          <div className="marquee-group flex items-center gap-20 pr-20 shrink-0" aria-hidden="true">
            {Techs.map((tech) => (
              <img key={`g3-${tech.id}`} src={tech.image} alt={tech.name} className="h-12 w-auto object-contain" />
            ))}
          </div>

          <div className="marquee-group flex items-center gap-20 pr-20 shrink-0" aria-hidden="true">
            {Techs.map((tech) => (
              <img key={`g4-${tech.id}`} src={tech.image} alt={tech.name} className="h-12 w-auto object-contain" />
            ))}
          </div>
